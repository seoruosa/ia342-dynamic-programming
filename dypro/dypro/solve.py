...
def solveStochastic(state, numberOfStages:int, finalStateCost, decision, solveInfeasibility, 
            transitionFunction, elementaryCost, initialFMap=dict(), initialPolicy=dict(), inf=np.inf):
    """ Solves the optimality recursive equation using a backward strategy
    ...
    for k in range(numberOfStages-1, -1, -1): #n-1 to 0
        for xk in state(k):
            ...
            for uk in decision(k):
                ...
                for wk in realizableRandomValues(k):
                    xk_next_maybe_infeasible = transitionFunction(k, xk, uk, wk)
                    ...
                    E_F_aux += elementaryCost(k, xk, uk) + _F
